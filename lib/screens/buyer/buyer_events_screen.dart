...
class BuyerEventsScreen extends StatefulWidget {
  ...
  const BuyerEventsScreen({super.key, required this.user});

  @override
  State<BuyerEventsScreen> createState() => _BuyerEventsScreenState();
}

class _BuyerEventsScreenState extends State<BuyerEventsScreen> {
  ...
  void _loadEvents() {
    ...
    _firestoreService.checkAndUpdatePendingEvents().then((_) {
      ...
      _eventsSubscription = _firestoreService.getEvents().listen((events) {
        if (mounted) {
          setState(() {
      ...
      developer.log('Error checking pending events: $error');
